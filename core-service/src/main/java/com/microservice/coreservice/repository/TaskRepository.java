...
public interface TaskRepository extends CrudRepository<Task, Long> {
    ...
    @Query("SELECT t FROM Task t WHERE t.projectId = :projectId AND t.sectionId = :sectionId")
    List<Task> getListTaskInSectionAndProject(Long projectId, Long sectionId);

    @Query(value = "SELECT t FROM Task t WHERE t.projectId IS NULL AND t.sectionId IS NULL")
